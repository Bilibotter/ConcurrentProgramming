...
public class ReentrantStampedLock {
    final ReentrantStampedLock.Sync sync;
    private final ReentrantStampedLock.WriteLock writerLock;
    private final ReentrantStampedLock.ReadLock readerLock;

    public ReentrantStampedLock() {
        this(false);
    }

    public ReentrantStampedLock(boolean fair) {
        sync = fair ? new ReentrantStampedLock.FairSync() : new ReentrantStampedLock.UnfairSync();
        writerLock = new ReentrantStampedLock.WriteLock(this);
        readerLock = new ReentrantStampedLock.ReadLock(this);
    }

    public ReentrantStampedLock.WriteLock writerLock() {
        return writerLock;
    }

    public ReentrantStampedLock.ReadLock readerLock() {
        return readerLock;
    }

    public static class WriteLock implements Lock {
        private final ReentrantStampedLock.Sync sync;

        public WriteLock(ReentrantStampedLock lock) {
            sync = lock.sync;
        }
    ...
    public static class ReadLock implements Lock {
        ...
        public ReadLock(ReentrantStampedLock lock) {
            sync = lock.sync;
        }
        ...
        public boolean validate(int stamp) {
            return sync.validate(stamp);
        }
    }

    static final class FairSync extends ReentrantStampedLock.Sync {
        @Override
    ...
    static final class UnfairSync extends ReentrantStampedLock.Sync {
    ...
    abstract static class Sync extends AbstractQueuedSynchronizer {
        ...
        // stamp clear if bigger than MAX
        // 读锁持有期间不能发生恰好等于MAX_STAMP N 倍的写操作
        private static final int MAX_STAMP = 1 << 30;
        ...
        protected boolean validate(int old) {
            return old == stamp;
        }
        ...
        protected boolean tryRelease(int arg) {
            ...
            if (writeState == 1) {
                ...
                // happens-before
                writeState--;
                stamp = stamp >= MAX_STAMP ? 0 : stamp+1;
